...
const { dialogGraphSchema } = await import(
  "../src/services/content-versioning.js"
);
...
test("daily signal ritual is deterministic, readable and user-specific", async () => {
  const { signalPatternFor } = await import("../src/services/signal-ritual.js");
  const first = signalPatternFor(10001, "2026-07-15");
  const repeated = signalPatternFor(10001, "2026-07-15");
  const otherResident = signalPatternFor(10002, "2026-07-15");
  assert.deepEqual(first, repeated);
  assert.equal(first.length, 5);
  assert.ok(first.every((value) => value === 0 || value === 1));
  assert.ok(new Set(first).size > 1);
  assert.notDeepEqual(first, otherResident);
});
