...
const schema=z.object({
  ...
  SENTRY_DSN:optionalUrl,SENTRY_TRACES_SAMPLE_RATE:z.coerce.number().min(0).max(1).default(.1),ENABLE_METRICS:bool(true),METRICS_TOKEN:z.string().min(16).optional(),APP_VERSION:z.string().default('2.0.0'),
  BACKUP_ENABLED:bool(false),BACKUP_DIR:z.string().default('/backups'),BACKUP_RETENTION_DAYS:z.coerce.number().int().min(2).max(365).default(14),BACKUP_HOUR_UTC:z.coerce.number().int().min(0).max(23).default(2),PRE_MIGRATION_BACKUP:bool(false),BACKUP_WEBHOOK_URL:optionalUrl,
