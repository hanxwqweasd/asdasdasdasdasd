...
async function renderSettings(){
  ...
  $('#content').innerHTML=`<div class="panel-grid"><article class="panel"><div class="panel-header"><div><h2>Аварийное управление</h2><p>Переключатели действуют сразу</p></div></div><form id="settingsForm" class="stack"><label class="check-row"><input name="maintenance_mode" type="checkbox" ${settings.maintenance_mode?'checked':''}> Режим технических работ</label><label>Заголовок экрана техработ<input name="maintenance_title" maxlength="120" value="${attr(settings.maintenance_title||'Дом закрыт на технические работы')}"></label><label>Сообщение игрокам<textarea name="maintenance_message" maxlength="700">${escapeHtml(settings.maintenance_message||'')}</textarea></label><label>Ожидаемое время / статус<input name="maintenance_eta" maxlength="160" value="${attr(settings.maintenance_eta||'Проверьте вход немного позже')}"></label><label>Ссылка поддержки<input name="maintenance_support_url" type="url" placeholder="https://t.me/username" value="${attr(settings.maintenance_support_url||'')}"></label><label class="check-row"><input name="expeditions_enabled" type="checkbox" ${settings.expeditions_enabled?'checked':''}> Разрешить вылазки</label><label class="check-row"><input name="notes_enabled" type="checkbox" ${settings.notes_enabled?'checked':''}> Разрешить записки</label><label class="check-row"><input name="shop_enabled" type="checkbox" ${settings.shop_enabled?'checked':''}> Разрешить магазин Stars</label><label>Комнат в одной вылазке<input name="max_expedition_rooms" type="number" min="1" max="20" value="${settings.max_expedition_rooms||6}"></label><button class="button primary" type="submit">Сохранить настройки</button></form></article><article class="panel"><div class="panel-header"><div><h2>Предпросмотр</h2><p>Так экран увидят игроки при следующем входе</p></div></div><div class="maintenance-preview"><span>ЛИФТ ВРЕМЕННО ОСТАНОВЛЕН</span><h3>${escapeHtml(settings.maintenance_title||'Дом закрыт на технические работы')}</h3><p>${escapeHtml(settings.maintenance_message||'')}</p><b>${escapeHtml(settings.maintenance_eta||'')}</b></div><pre class="code">${json(settings)}</pre></article></div>`;
  $('#settingsForm').addEventListener('submit',async event=>{event.preventDefault();const d=formObject($('#content'));await api('/settings',{method:'PATCH',body:{maintenance_mode:Boolean(d.maintenance_mode),maintenance_title:d.maintenance_title,maintenance_message:d.maintenance_message,maintenance_eta:d.maintenance_eta,maintenance_support_url:d.maintenance_support_url||'',expeditions_enabled:Boolean(d.expeditions_enabled),notes_enabled:Boolean(d.notes_enabled),shop_enabled:Boolean(d.shop_enabled),max_expedition_rooms:Number(d.max_expedition_rooms)}});toast('Настройки применены');await renderSettings();updateHeader();});
}
